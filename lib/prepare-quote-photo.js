...
export async function prepareQuotePhoto(file) {
  ...
  if (isHeic) {
    let timeout;
    try {
      // Lazy-load the decoder only for HEIC; no third-party upload is needed.
      imageFile = await Promise.race([
        import("heic-to/csp").then(({ heicTo }) => heicTo({ blob: file, type: "image/jpeg", quality: 0.9 })),
        new Promise((_, reject) => { timeout = setTimeout(() => reject(new Error("Conversion timed out")), 45000); })
      ]);
    } catch {
      throw new Error(`${file.name}: this HEIC photo could not be converted. Try a JPG copy or another photo. Your other photos are still selected.`);
    } finally {
      clearTimeout(timeout);
    }
  }
  if (!isHeic && !/^image\/(jpeg|png|webp|gif)$/.test(file.type) && !/\.(jpe?g|png|webp|gif)$/i.test(file.name)) {
